...
test('getNormalizedSpectrum no filter', () => {
...
test('getNormalizedSpectrum normalize', () => {
  const normalized = getNormalizedSpectrum(spectrum, {
    filters: [{ name: 'normed' }],
  });
...
test('getNormalizedSpectrum divideByMax', () => {
  const normalized = getNormalizedSpectrum(spectrum, {
...
test('getNormalizedSpectrum divideBySD', () => {
  const normalized = getNormalizedSpectrum(spectrum, {
...
test('getNormalizedSpectrum centerMean', () => {
  const normalized = getNormalizedSpectrum(spectrum, {
...
test('getNormalizedSpectrum rescale', () => {
  const normalized = getNormalizedSpectrum(spectrum, {
...
test('getNormalizedSpectrum ensureGrowing', () => {
  const spectrum = {
    variables: {
      x: {
...
test('getNormalizedSpectrum applyRangeSelectionFirst', () => {
  const spectrum = {
    variables: {
      x: {
        data: [1, 2, 3, 4, 5],
